...
- **Main-process libraries must be CJS-compatible.** electron-vite bundles the main process to CommonJS. ESM-only packages (anything with `"type": "module"` and no CJS export, common for newer Sindre Sorhus utilities like `p-retry` v5+, `chalk` v5+, `nanoid` v4+) compile cleanly but resolve `default` as `{default: fn}` at runtime, so the wrapped function ends up as `TypeError: x is not a function` the first time it's called. Before adding a main-process dep: check `node_modules/<pkg>/package.json` for `"type": "module"` without an `"exports"` map pointing at a `.cjs` file, and pick the last CJS-compatible major if so (e.g. `p-retry@^4`, `chalk@^4`). Renderer-side deps are unaffected — Vite handles ESM natively for the web bundle.
